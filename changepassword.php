...
if (empty($_SESSION['email']) && empty($_COOKIE['fname']) && empty($_COOKIE['sname'])) {
	...
	$sname = $_COOKIE['sname'];
	$email = $_COOKIE['email'];
	$emailErr = $newpasswordErr = $oldpasswordErr = $booking = '';
	$method = "AES-256-ECB";
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		if(empty($emailErr) && empty($oldpasswordErr) && empty($newpasswordErr)){
			try{
				$query = $conn -> prepare("SELECT email, password  FROM members WHERE email=:email");
				...
				if(empty($result['email'])){
					...
					try{
						...
						try{
							...
							$mail->Body    = "<div class='clientmail'>Password changed successfully.</div>";
